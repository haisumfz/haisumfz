...
# ===== 扩展布局宽度 —— 解决“中间空太多”问题 =====
st.set_page_config(layout="wide")  # 可选：启用宽屏模式（推荐配合下面的CSS）
...
def create_3d_model_plot(model_data, selected_element_id=None, selected_node_id=None):
    """创建3D模型图"""
    fig = go.Figure()
    ...
    for elem in model_data['elements']:
        if elem['type'] == 'column':
            x_coords = [elem['start_coord'][0], elem['end_coord'][0]]
            y_coords = [elem['start_coord'][1], elem['end_coord'][1]]
            z_coords = [elem['start_coord'][2], elem['end_coord'][2]]

            # 高亮选中的单元
            line_color = 'orange' if elem['id'] == selected_element_id else 'blue'
            line_width = 15 if elem['id'] == selected_element_id else 10
            marker_size = 5 if elem['id'] == selected_element_id else 3
            marker_color = 'orange' if elem['id'] == selected_element_id else 'blue'
            ...
            fig.add_trace(go.Scatter3d(
                ...
                name=f'柱 {elem["id"]}',
                text=[f'柱 {elem["id"]} ({elem["start"]}-{elem["end"]})'],
                hovertemplate='<b>%{text}</b><br>' +
                              '起点: (%{x:.2f}, %{y:.2f}, %{z:.2f})<br>' +
                              '终点: (%{x:.2f}, %{y:.2f}, %{z:.2f})<extra></extra>',
                customdata=[elem['id']]
            ))
    ...
    for elem in model_data['elements']:
        if elem['type'] == 'beam':
            ...
            # 高亮选中的单元
            line_color = 'orange' if elem['id'] == selected_element_id else 'red'
            line_width = 15 if elem['id'] == selected_element_id else 10
            marker_size = 5 if elem['id'] == selected_element_id else 3
            marker_color = 'orange' if elem['id'] == selected_element_id else 'red'
            ...
            fig.add_trace(go.Scatter3d(
                ...
                marker=dict(size=marker_size, color=marker_color),
                name=f'梁 {elem["id"]}',
                text=[f'梁 {elem["id"]} ({elem["start"]}-{elem["end"]})'],
                hovertemplate='<b>%{text}</b><br>' +
                              '起点: (%{x:.2f}, %{y:.2f}, %{z:.2f})<br>' +
                              '终点: (%{x:.2f}, %{y:.2f}, %{z:.2f})<extra></extra>',
                customdata=[elem['id']]
    ...
    # 绘制梁编号和规格
    beam_x, beam_y, beam_z, beam_text = [], [], [], []
    for elem in model_data['elements']:
        if elem['type'] == 'beam':
            mid_x = (elem['start_coord'][0] + elem['end_coord'][0]) / 2
            mid_y = (elem['start_coord'][1] + elem['end_coord'][1]) / 2
            mid_z = (elem['start_coord'][2] + elem['end_coord'][2]) / 2
            # 将标签稍微偏移，避免与构件重叠
            beam_x.append(mid_x)
            beam_y.append(mid_y)
            beam_z.append(mid_z + 0.2)  # 在Z方向偏移
            # 获取梁规格和荷载
            beam_spec = st.session_state.analyzer_state['element_steels'].get(elem['id'], "HW200×200×8×12")
            load_value = st.session_state.analyzer_state['beam_loads'].get(elem['id'], 5.0)
            total_load = load_value
            if st.session_state.analyzer_state['current_analysis_data'] and st.session_state.analyzer_state[
                'current_analysis_data'].get('include_self_weight', False):
                _, _, _, beam_weight = get_steel_properties(beam_spec, st.session_state.analyzer_state['h_steel_data'])
                beam_self_weight = beam_weight * 9.81 / 1000
                total_load += beam_self_weight
            beam_text.append(f'梁 {elem["id"]}\n{beam_spec}\n{total_load:.2f}kN/m')
    ...
    for elem in model_data['elements']:
        if elem['type'] == 'column':
            ...
            mid_z = (elem['start_coord'][2] + elem['end_coord'][2]) / 2
            # 将标签稍微偏移，避免与构件重叠
            col_x.append(mid_x + 0.3)  # 在X方向偏移
            col_y.append(mid_y)
            col_z.append(mid_z)
            # 获取柱规格
            col_spec = st.session_state.analyzer_state['element_steels'].get(elem['id'], "HW200×200×8×12")
            col_text.append(f'柱 {elem["id"]}\n{col_spec}')
    ...
    for key, val in model_data['nodes'].items():
        node_id, x, y, z = val
        node_x.append(x)
        node_y.append(y)
        node_z.append(z)
    ...
    node_colors = []
    node_sizes = []
    for node_id, x, y, z in zip(node_x, node_x, node_y, node_z):
        # 找到节点ID
        for key, val in model_data['nodes'].items():
            if val[1] == x and val[2] == y and val[3] == z:
                current_node_id = val[0]
                break

        if current_node_id == selected_node_id:
    ...
    # 绘制节点编号（偏移到左上侧，避免被梁柱遮挡）
    node_label_x, node_label_y, node_label_z, node_label_text = [], [], [], []
    for key, val in model_data['nodes'].items():
        node_id, x, y, z = val
        node_label_x.append(x - 0.3)  # X方向偏移
        node_label_y.append(y)
        node_label_z.append(z + 0.3)  # Z方向偏移
        node_label_text.append(f'{node_id}')

    fig.add_trace(go.Scatter3d(
        x=node_label_x, y=node_label_y, z=node_label_z,
        mode='text',
    ...
    # 绘制支座
    support_x, support_y, support_z, support_text = [], [], [], []
    for node_id in model_data['supports']:
        for key, val in model_data['nodes'].items():
            if val[0] == node_id:
                support_x.append(val[1])
                support_y.append(val[2])
                support_z.append(val[3])
                support_text.append(f'{node_id}')
                break
    ...
    # 计算边界以适配视图
    all_x = [x for x, _, _ in [elem['start_coord'] for elem in model_data['elements']] + [elem['end_coord'] for elem in
                                                                                          model_data['elements']]]
    all_y = [y for _, y, _ in [elem['start_coord'] for elem in model_data['elements']] + [elem['end_coord'] for elem in
                                                                                          model_data['elements']]]
    all_z = [z for _, _, z in [elem['start_coord'] for elem in model_data['elements']] + [elem['end_coord'] for elem in
                                                                                          model_data['elements']]]

    # 坐标范围（保留原始数据范围，但 Y 给一点空间）
    x_range = [min(all_x) - 10, max(all_x) + 10]
    z_range = [min(all_z) - 1, max(all_z) + 1]
    y_range = [-5, 5]  # 避免 Y 轴 collapse

    x_center = (x_range[0] + x_range[1]) / 2
    z_center = (z_range[0] + z_range[1]) / 2

    # 相机：只需方向正确（从 -Y 方向看），距离任意
    camera_x = x_center
    camera_y = -10  # 只要 <0 即可，值大小不影响正交投影
    camera_z = z_center
    ...
    fig.update_layout(
        ...
        height=700,
        hovermode='closest',
        showlegend=False  # ←←← 关键修改：隐藏图例
    )
    # print(f"DEBUG: scene_scale={scene_scale:.1f} m, camera_distance={camera_distance:.1f} m")
    return fig
...
def display_results():
    ...
    # 柱内力 - 默认折叠显示
    with st.expander("**柱内力**", expanded=False):
    ...
    # 梁内力 - 默认折叠显示
    with st.expander("**梁内力**", expanded=False):
    ...
    # 节点位移 - 默认折叠显示
    with st.expander("**节点位移**", expanded=False):
        for node_tag in ops.getNodeTags():
            try:
...
def perform_verification():
    ...
    try:
        ...
        # 梁验算 - 折叠显示
        with st.expander("**梁验算（强度）**", expanded=False):
            for i, ele_id in enumerate(st.session_state.analyzer_state['beam_elements']):
                try:
                    forces = get_element_forces(ele_id)
        ...
        # 柱验算 - 折叠显示
        with st.expander("**柱验算（平面内 + 平面外稳定 + 长细比）**", expanded=False):
            height_list = st.session_state.analyzer_state['current_analysis_data']['heights']
            net_to_gross_ratio = 0.9  # 示例值，请根据实际情况调整或从状态中读取
            fy = 235  # 钢材屈服强度 (MPa)，请根据项目设定
            f = fy  # 设计强度，简化处理；实际应除以分项系数，如 f = fy / 1.1
            column_in_plane_k = 1.0  # 平面内计算长度系数
            column_out_plane_k = 1.0  # 平面外计算长度系数
            ...
            for i, ele_id in enumerate(st.session_state.analyzer_state['column_elements']):
                try:
                    ...
                    # 长细比限值验算（按 GB 50017，主要受压构件 λ ≤ 150）
                    slenderness_limit = 150
                    slenderness_ok_x = lambda_x <= slenderness_limit
                    slenderness_ok_y = lambda_y <= slenderness_limit
                    slenderness_ok = slenderness_ok_x and slenderness_ok_y
                    ...
                    # 内力（单位转换）
                    N = axial_force_kN * 1000  # N
                    M = max_moment_kN_m * 1e6  # N·mm

                    # 平面内稳定验算（GB 50017 式 7.2.1-1）
                    beta_mx = 1.0
                    gamma_x = 1.05
                    E = 2.06e5  # MPa
                    ...
                    # 平面外稳定验算（GB 50017 式 7.2.2）
                    phi_b = 1.0  # 简化：无侧向支撑时取1.0，实际应按规范计算
                    eta = 0.0
                    beta_ty = 1.0
                    ...
                    with st.expander(f"柱 {i + 1} (单元 {ele_id}, {col_spec})"):
                        ...
                        # 长细比验算结果
                        st.write(f"  - 长细比限值: {slenderness_limit}")
                        if slenderness_ok:
                            st.success("  - 长细比验算: 通过 ✓")
                        else:
                            st.error(f"  - 长细比验算: 不通过！✗ (λx={lambda_x:.1f}, λy={lambda_y:.1f})")

                        st.write(f"  - 平面内稳定比: {ratio_in:.3f}")
                        st.write(f"  - 平面外稳定比: {ratio_out:.3f}")

                        # 综合判断
                        stable_ok = (ratio_in <= 1.0) and (ratio_out <= 1.0)
                        if stable_ok and slenderness_ok:
                            st.success("  - 综合验算结果: 通过 ✓")
                        else:
                            st.error("  - 综合验算结果: 不通过！✗")
...
def main():
    ...
    if st.session_state.model_data['elements']:
        # 使用扩展列布局，让模型区域更大
        col1, col2 = st.columns([3, 2])  # 模型区域占3份，参数修改区域占2份
        ...
        with col1:
            ...
            # 创建3D模型图
            fig_3d = create_3d_model_plot(st.session_state.model_data,
